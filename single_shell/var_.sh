...
[  -f  /tmp/ha_ip.txt ] && {
grep   '\[master-ha\]'  /etc/ansible/hosts || sed '/^\s*$/d'   /tmp/ha_ip.txt | grep  "^#"   -v |sed  '1 i\[master-ha]'    >> /etc/ansible/hosts 
}
###
##note节点ssh优化
#
...
- hosts: [slave]
  tasks:
    - name: 1拷贝安装文件到slave节点<a01>
      copy: src=/root/K8s/Software_package/flannel-v0.11.0-linux-amd64.tar.gz  dest=/root/K8s/Software_package  force=no
    - name: 2拷贝安装文件到slave节点<a01>
      copy: src=/root/K8s/Software_package/etcd-v3.2.28-linux-amd64.tar.gz  dest=/root/K8s/Software_package  force=no
...
#开启master-HA
master_ha_list=$(ansible  master-ha  -m ping |grep  SUCCESS|awk   '{print $1}' |wc -l)
master_ha_list02=$(ansible  master-ha  -m ping |grep  SUCCESS|awk   '{print $1}')
[[ "$master_ha_list"   -ge   "1" ]]  &&  {
#ip处理
master_ha_re="$(ansible  master-ha,master  -m ping |grep  SUCCESS|awk   '{print $1}')"
...
#master-HA
ansible master-ha -m script -a "chdir=/tmp /root/K8s/Cluster_shell_yaml/master-HA.sh "
sleep 5
            ...
            then
                    case $OPTION in
                      ...
                      password_=$(whiptail --title "#请输入本机root密码 并回车#" --passwordbox "确定提交?" 10 60 3>&1 1>&2 2>&3)
                       ...
                       whiptail --title "kubernetes_v1.15.7单机版安装完毕" --msgbox "K8s单机版安装完毕,web控制界面dashboard地址为： http://IP:42345 \n内网yum网址为:http://IP:42344  \
                         ...
                         #设置默认动态存储
                          kubectl patch storageclass gluster-heketi -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"true"}}}'
                            ...
                            # [ $?  -ne 0 ] && echo "k8s集群部署完毕，持久化部署条件不满足，k8s集群持久化部署失败"
                             whiptail --title "kubernetes_v1.15.7集群版安装完毕" --msgbox "K8s集群版安装完毕,web控制界面dashboard地址为： http://IP:42345 \n内网yum网址为:http://IP:42344  \
